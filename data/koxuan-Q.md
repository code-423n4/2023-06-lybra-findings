...
96:         for (uint i = 0; i < _pools.length; i++) {

140:         for (uint i = 0; i < pools.length; i++) {
...
156:         uint256 etherInLp = (IEUSD(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2).balanceOf(ethlbrLpToken) * uint(etherPrice)) / 1e8;

237:             rewardRatio = (amount + remainingRewards) / duration;
...
144:                 borrowed = borrowed * (1e20 + peUSDExtraRatio) / 1e20;
...
157:         uint256 lbrInLp = (IEUSD(LBR).balanceOf(ethlbrLpToken) * uint(lbrPrice)) / 1e8;

157:         uint256 lbrInLp = (IEUSD(LBR).balanceOf(ethlbrLpToken) * uint(lbrPrice)) / 1e8;

159:         return (userStaked * (lbrInLp + etherInLp)) / totalLp;

159:         return (userStaked * (lbrInLp + etherInLp)) / totalLp;

159:         return (userStaked * (lbrInLp + etherInLp)) / totalLp;

171:         return rewardPerTokenStored + (rewardRatio * (lastTimeRewardApplicable() - updatedAt) * 1e18) / totalStaked();
...
184:         return 100 * 1e18 + redemptionBoost + esLBRBoost.getUserBoost(_account, userUpdatedAt[_account], finishAt);
...
192:         return (stakedLBRLpValue(user) * 10000) / stakedOf(user) < 500;

192:         return (stakedLBRLpValue(user) * 10000) / stakedOf(user) < 500;

213:             uint256 biddingFee = (reward * biddingFeeRatio) / 10000;

213:             uint256 biddingFee = (reward * biddingFeeRatio) / 10000;

216:                 biddingFee = biddingFee * uint256(lbrPrice) / 1e8;

216:                 biddingFee = biddingFee * uint256(lbrPrice) / 1e8;

234:             rewardRatio = amount / duration;

236:             uint256 remainingRewards = (finishAt - block.timestamp) * rewardRatio;
...
242:         finishAt = block.timestamp + duration;
...
97:             require(configurator.mintVault(_pools[i]), "NOT_VAULT");

103:         require(_biddingRatio <= 8000, "BCE");

108:         require(ratio <= 1e20, "BCE");

113:         require(ratio <= 1e20, "BCE");

122:         require(finishAt < block.timestamp, "reward duration not finished");

196:         require(!isOtherEarningsClaimable(msg.sender), "Insufficient DLP, unable to claim rewards");

206:         require(isOtherEarningsClaimable(user), "The rewards of the user cannot be bought out");

208:             require(isEUSDBuyoutAllowed, "The purchase using EUSD is not permitted.");

218:                 require(success, "TF");

232:         require(amount > 0, "amount = 0");

240:         require(rewardRatio > 0, "reward ratio = 0");
